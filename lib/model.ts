...
export const rowSchema = productSchema.extend({ weight: z.string(), price: z.string(), currency: z.enum(currencies), leadTime: z.string() });
export const editorSchema = z.object({ date: z.string(), serial: z.string(), customer: z.string(), email: z.string(), payment: z.string(), incoterms: z.string(), validity: z.string(), origin: z.string(), rates: z.object({ EUR: z.string(), GBP: z.string() }), products: z.array(rowSchema).min(1) });
...
export function newEditor(data: Imported, serial: number, rates = { EUR: '', GBP: '' }): Editor { return { ...defaults, date: todayDubai(), serial: String(serial), customer: '', email: '', rates, products: data.products.map(p => ({ ...p, weight: p.unit_weight_kg === null ? '' : String(p.unit_weight_kg), price: '', currency: 'USD', leadTime: '' })) }; }
